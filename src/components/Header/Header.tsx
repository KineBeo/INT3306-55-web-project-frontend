      ...
      return (
        <div
          className={`absolute inset-x-0 top-0 transition-all will-change-[transform,opacity] ${
            showHeaderSearch ? "visible" : "-translate-x-0 -translate-y-[90px] scale-x-[0.395] scale-y-[0.6] opacity-0 invisible pointer-events-none"
          }`}
        >
    ...
    <>
      ...
      <header ref={headerInnerRef} className={`sticky top-0 z-40 ${className}`}>
        <div
          ...
          ${showHeaderSearch ? "duration-75 scale-y-[4.4]" : ""}`}
        ></div>
